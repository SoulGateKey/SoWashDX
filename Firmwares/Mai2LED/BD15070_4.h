...
void setLedGs8Bit() {
  leds[(req.index+1)*2-1] = CRGB(req.color[0], req.color[1], req.color[2]);
  leds[(req.index+1)*2-2] = CRGB(req.color[0], req.color[1], req.color[2]);
  ack_init();
}
...
void setLedGs8BitMulti() {
  ...
  leds(req.start, /*req.end*/16 - 1) = StartFadeColor;
  NeedFade = false;
  ack_init();
}
...
#if NUM_LEDS > 8
  leds[16] = blend(0x000000, 0xFFFFFF, req.BodyLed);
  leds[17] = blend(0x000000, 0xFFFFFF, req.ExtLed);    //same as BodyLed
  leds[18] = blend(0x000000, 0xFFFFFF, req.SideLed);  //00 or FF
  FastLED.show();
#endif
  ack_init();
}
